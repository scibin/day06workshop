...
export class ShowComponent implements OnInit {
  ...
  constructor(
    ...
    private weatherSvc: WeatherServiceService) { }
  ...
  convertWeatherData(data: Weather) {
    let tempContainer = data.temp - 273.15;       // Converts from K to degC
    data.temp = parseFloat(tempContainer.toFixed(2));
    let pressureContainer = data.pressure / 1000; // Converts from hPa to bar
